...
# def ht_k_curve(y_true, y_pred,max_k):
# 	recommend = np.argsort(-y_pred)
# 	res = []
# 	for k in range(max_k):
		
def hr_k_curve(y_true, y_pred, max_k):
	recommend = np.argsort(-y_pred)
	res = []
	for k in range(max_k):
		res.append(np.sum(y_true[recommend[:k+1]]))
	return res
...
def dcg_score(y_true, y_score, k=5):
  ...
  ##############idcg
  # idcg = sorted(y_true, reverse = True)
  # idcg = 2**np.array(idcg)-1
  # idcg = np.sum(idcg/np.log2(np.arange(len(y_true))+2))
